...
import { dictionaries } from "@jdm-pro/lookup";
...
export function makerDedupeKey(value) {
  return normalizeDisplayText(value).toLowerCase().replace(/[・･]/g, "").replace(/\s+/g, "");
}

export function canonicalMakerValue(label) {
...
export async function fetchMakerOptions({ fetchImpl = fetch, sites = Object.keys(adapters) } = {}) {
  ...
  for (const [index, result] of results.entries()) {
    if (result.status === "rejected") {
      debugLog("crawler.makers.fetch.site.error", {
        site: sites[index],
