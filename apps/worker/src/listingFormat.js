...
export function titleLine(row) {
  return [row.modelYear, titleCase(row.maker), titleCase(row.model)].filter(Boolean).join(" ").trim();
}

// One caption used by both Telegram (html) and Instagram (plain + hashtags).
export function buildCaption(listing, { html = false, hashtags = false } = {}) {
...
export function buildKeyboard({ listingId, autoCreateVehicles, vehicleUrl }) {
  const vehicleButton =
